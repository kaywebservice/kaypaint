...
export function activateTool(ctx: ToolCtx) {
  ...
  const down = (e: any) => {
    ...
    // Spot heal: source offset is scanned tightly around the blemish.
    const r = Math.max(1, (Number(ctx.get("size") ?? 14) / 2) / Math.abs(obj.scaleX || 1));
  ...
  const move = (e: any) => {
    ...
    stroke.lastX = lp.x;
    stroke.lastY = lp.y;
    const r = Math.max(1, (Number(ctx.get("size") ?? 14) / 2) / scale);
