...
export function activateTool(ctx: ToolCtx) {
  ...
  const down = (e: any) => {
    ...
    if (e.e.altKey) {
      // Pick the clone source (in the object's local pixel space).
      srcX = lp.x;
      srcY = lp.y;
      return;
    }
    if (srcX === null || srcY === null) {
      notify("Alt-click to pick the clone source first.", "warning");
      return;
    }
    ...
    stampInto(img, w, h, lp.x, lp.y, r, srcX, srcY);
    stroke.dirty = true;
    sync(stroke, canvas);
  };
  ...
  const move = (e: any) => {
    ...
    stampInto(stroke.img, stroke.w, stroke.h, lp.x, lp.y, r, stroke.srcX, stroke.srcY);
    stroke.dirty = true;
    const now = Date.now();
...
/** Copy the source disc over the destination disc with a soft edge. */
function stampInto(
  img: ImageData,
  w: number,
  h: number,
  ...
  for (let y = y0; y <= y1; y++) {
    for (let x = x0; x <= x1; x++) {
      ...
      const a = f * f * (2 - f);
      const sx = Math.min(w - 1, Math.max(0, x + (srcX - cx)));
      const sy = Math.min(h - 1, Math.max(0, y + (srcY - cy)));
      const si = (sy * w + sx) * 4;
      const di = (y * w + x) * 4;
      for (let ch = 0; ch < 4; ch++) {
        data[di + ch] = data[di + ch] * (1 - a) + data[si + ch] * a;
      }
    }
  }
}